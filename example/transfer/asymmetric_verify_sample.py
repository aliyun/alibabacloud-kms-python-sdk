...
def new_user_asymmetric_verify_sample():
    ...
    asymmetric_verify(client)


# 密钥迁移前示例代码
def before_migrate_asymmetric_verify_sample():
...
# 密钥迁移后示例代码
def after_migrate_asymmetric_verify_sample():
    # 创建kms共享网关config并设置相应参数
    config = open_api_models.Config(
        # 设置地域Id
        region_id='<your-region-id>',
...
def asymmetric_verify(client):
    ...
    try:
        response = client.asymmetric_verify_with_options(request, runtime)
        print(str(response.body))
    except Exception as e:
        print(str(e))


new_user_asymmetric_verify_sample()
before_migrate_asymmetric_verify_sample()
after_migrate_asymmetric_verify_sample()
