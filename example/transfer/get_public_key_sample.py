...
def new_user_get_public_key_sample():
...
# 密钥迁移前示例代码
def before_migrate_get_public_key_sample():
...
# 密钥迁移后示例代码
def after_migrate_get_public_key_sample():
    # 创建kms共享网关config并设置相应参数
    config = open_api_models.Config(
        # 设置地域Id
        region_id='<your-region-id>',
...
before_migrate_get_public_key_sample()
after_migrate_get_public_key_sample()
