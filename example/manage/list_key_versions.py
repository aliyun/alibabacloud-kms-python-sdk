...
class ListKeyVersions:
    ...
    def main(
        ...
        open_api_config = ListKeyVersions.create_open_api_config(EnvClient.get_env('ALIBABA_CLOUD_ACCESS_KEY_ID'), EnvClient.get_env('ALIBABA_CLOUD_ACCESS_KEY_SECRET'), 'your region id')
    ...
    async def main_async(
        ...
        # 请确保代码运行环境设置了环境变量 ALIBABA_CLOUD_ACCESS_KEY_ID 和 ALIBABA_CLOUD_ACCESS_KEY_SECRET。
        # 工程代码泄露可能会导致 AccessKey 泄露，并威胁账号下所有资源的安全性。以下代码示例使用环境变量获取 AccessKey 的方式进行调用，仅供参考，建议使用更安全的 STS 方式，更多鉴权访问方式请参见：https://help.aliyun.com/document_detail/378657.html
        open_api_config = await ListKeyVersions.create_open_api_config_async(EnvClient.get_env('ALIBABA_CLOUD_ACCESS_KEY_ID'), EnvClient.get_env('ALIBABA_CLOUD_ACCESS_KEY_SECRET'), 'your region id')
        client = await ListKeyVersions.create_client_async(open_api_config)
        page_number = NumberClient.parse_int(UtilClient.assert_as_string('your pageNumber'))
        key_id = 'your keyId'
        page_size = NumberClient.parse_int(UtilClient.assert_as_string('your pageSize'))
        response = await ListKeyVersions.list_key_versions_async(client, page_number, key_id, page_size)
        print(response)
