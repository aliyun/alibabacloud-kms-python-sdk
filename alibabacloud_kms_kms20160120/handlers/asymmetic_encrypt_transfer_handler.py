...
class AsymmetricEncryptTransferHandler(KmsTransferHandler):
    ...
    def build_kms_request(self, request: open_api_models.OpenApiRequest, runtime_options: KmsRuntimeOptions):
        if not request.query.get('Plaintext'):
        ...
        kms_request.plaintext = base64.b64decode(request.query.get('Plaintext'))
    ...
    def transfer_response(self, response, runtime_options: KmsRuntimeOptions) -> dict:
        ...
        body = {
            ...
            'CiphertextBlob': base64.b64encode(response.ciphertext_blob).decode(encoding),
